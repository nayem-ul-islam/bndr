...
class _LoginScreenState extends State<LoginScreen> {
    ...
    if (res.size == 0) {
      ...
      res.docs.forEach((res) async {
        setState(() {
          id = res.id;
          name = res.data()['_name'];
          phoneNumber = res.data()['_phone'];
          bndrId = res.data()['bndr_id'];
          guideBookNo = res.data()['_guide_book_no'];
          centerId = res.data()['_idby_center'];
  ...
  void _update() async {
    firestoreInstance.collection('snagar version 2 ').doc(id).set(
        {'_phone': c1.text, 'password': ps1.text},
                    ...
                    child: Column(
                      children: [
                        ...
                        InputTaker(
                          ...
                          hints: language ? centerId : centerId,
                          phnx: c2,
                        ),
                        SizedBox(
                          height: 5,
                                ...
                                if (ps1.text.length >= 6 &&
                                  ...
                                  Get.snackbar(
                                      language
                                          ...
                                          : 'কিছু ভুল হয়েছে',
                                      language
                                          ? 'Your password must be 6-15 characters'
                                          : 'আপনার পাসওয়ার্ড ৬-১৫ অক্ষরের হতে হবে');
                                }
                              } else {
                                Get.snackbar(
                                    language
